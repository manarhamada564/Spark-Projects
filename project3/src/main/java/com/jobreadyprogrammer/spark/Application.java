...
//		    philDf.show(10);
		    ...
		    //method to combine 2 files format 
	...
	private static void combineDataframes(Dataset<Row> df1, Dataset<Row> df2) {
		// Match by column names using the unionByName() method doesn't matter columns order
...
//		df = df.repartition(5);
	...
	public static Dataset<Row> buildDurhamParksDataFrame(SparkSession spark){
		...
		df = df.withColumn("park_id", concat(df.col("datasetid"), lit("_"), 
				...
				.withColumn("geoX", df.col("geometry.coordinates").getItem(0)) //x : first element in array
